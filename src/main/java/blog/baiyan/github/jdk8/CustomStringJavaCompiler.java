...
public class CustomStringJavaCompiler {
     ...
     */
    public void runMainMethod(String methodName,Object ... params) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, InstantiationException {
        PrintStream out = System.out;
        // 判断传递方法是否非空
        if(StringUtils.isEmpty(methodName)){
            log.error("传递方法为空.");
            throw new NullPointerException("传递方法为空");
        }
        try {
            long startTime = System.currentTimeMillis();
            ...
            if(CollectionUtils.isEmpty(Arrays.asList(methods))){
                log.error("对象方法为空.");
                throw new NullPointerException("对象方法为空");
            }
            for (Method method : methods) {
                ...
                if(Objects.equals(method.getName(),methodName)){
                    ...
                    if(CollectionUtils.isEmpty(Arrays.asList(parameterTypes))){
                        log.warn("方法参数为空.");
                        Method declaredMethod = aClass.getDeclaredMethod(methodName);
                        declaredMethod.invoke(o);
                    }
                    if(parameterTypes.length != params.length){
                        log.error("当前方法参数长度与传递参数长度不符合。");
                        throw new IllegalAccessException("当前方法参数长度与传递参数长度不符合.");
                    }
                    Class<?>[] paramsTypes = new Class[params.length];
                    for (int i = 0; i < parameterTypes.length; i++) {
                        paramsTypes[i] = params[i].getClass();
                    }
                    // 将对象方法的所需参数类型和根据传递方法的所需参数类型对比
                    boolean bool = Arrays.equals(parameterTypes, paramsTypes);
                    if(bool){
                        log.info("true");
